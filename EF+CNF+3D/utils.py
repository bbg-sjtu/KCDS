...
def plot_potential_velocity_field(model, limit, positions, savepath=None):
    # 创建网格
    x_grid = np.linspace(limit[0, 0], limit[1, 0], 100)
    y_grid = np.linspace(limit[0, 1], limit[1, 1], 100)
    ...
    with torch.no_grad():
        ...
        potential = torch.norm(diff, dim=1).cpu().numpy()

        # 计算速度场
        velocities = model.direct_velocity_prediction(grid_tensor, target_tensor).cpu().numpy()

    reproduction=model.inference(startpoints, target_tensor, dt=0.05, steps=200)
    ...
    ax2.streamplot(X, Y, U, V, density=1.5, color='black', linewidth=1, arrowsize=1.5)
    ...
    # 为2D图添加colorbar，使用与第一个colorbar相同的参数
    cbar2 = plt.colorbar(contour, ax=ax2, shrink=0.6, aspect=20, pad=0.1)
    cbar2.set_label('Potential', rotation=270, labelpad=15)
