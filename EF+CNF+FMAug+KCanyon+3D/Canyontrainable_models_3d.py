...
from YspaceKmeans import *
from KCanyon3DField import KCanyon3D
...
class SDSEF_FM_KCanyonTrainable_3D(nn.Module):
    """
    使用流匹配的欧几里得化流的稳定动力系统
    """
    def __init__(self, dim, hidden_dim, num_layers, K, SDSEF_FM_modelpath, x0s, x_star):
        super(SDSEF_FM_KCanyonTrainable_3D, self).__init__()
        self.dim = dim
        self.K = K
    ...
    def initKCanyonField(self,x0s,x_star):
        ...
        # 在y空间进行kmeans聚类
        angles = yspace_kmeans_3d(y_star, y0s, self.K)
        # self.CanyonVF = KCanyon_Combined_Field(angles, a_init=1.0, b_init=10.0)
        self.CanyonVF = KCanyon3D(angles[0], a_init=1.0, b_init=10.0)
