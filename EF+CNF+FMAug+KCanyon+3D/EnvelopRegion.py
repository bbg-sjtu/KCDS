...
class TrajectoryRegionClassifier:
    ...
    def sample_outer_points(self, batch_size, limit):
        ...
        while len(points) < batch_size:
            # 在整个工作空间采样
            x = np.random.uniform(low=[limit[0,0], limit[0,1]], high=[limit[1,0], limit[1,1]], size=(1000, 2))
    ...
    def sample_source_points(self, batch_size, limit):
        points = np.random.uniform(low=[limit[0,0], limit[0,1]], high=[limit[1,0], limit[1,1]], size=(batch_size, 2))
        return torch.from_numpy(points)
...
if __name__ == "__main__":
    ...
    import scipy.io
    filepath = "experiment_results/AlgorithmFig/traj.mat"
    mat_data = scipy.io.loadmat(filepath)
    positions=mat_data['traj']
    x_min, x_max = positions[:, 0].min() - 0.2, positions[:, 0].max() + 0.2
    y_min, y_max = positions[:, 1].min() - 0.2, positions[:, 1].max() + 0.2
    ...
    plt.legend()
    # plt.show()
    plt.savefig(f'experiment_results/AlgorithmFig/InOut_ER.png', dpi=300)
    print('yes')