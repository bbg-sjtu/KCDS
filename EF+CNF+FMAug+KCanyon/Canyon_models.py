# 相比于之前的SDSEF_FM，只在计算y空间速度时改成了KCanyonPotential计算方式
# x和y空间的映射保持不变，依然是由专家轨迹场和简单标准场训练得到
# 然后将简单标准场直接改为KCanyon的收缩形式，应用同样的映射得到具备收缩行为的x空间场
...
class SDSEF_FM_KCanyon(nn.Module):
    """
    使用流匹配的欧几里得化流的稳定动力系统
    """
    def __init__(self, dim, hidden_dim=128, num_layers=4, K=1, a=1.0, b=10.0):
        super(SDSEF_FM_KCanyon, self).__init__()
        self.dim = dim
    ...
    def initKCanyonField(self,x0s,x_star):
        # 将x0和x*映射到y空间
        y0s = self.diffeo.differentiable_forward(x0s).detach().cpu().numpy()
        y_star = self.diffeo.differentiable_forward(x_star).detach().cpu().numpy()
        # 在y空间进行kmeans聚类
        angles = yspace_kmeans(y_star, y0s, self.K)
        self.CanyonVF = KCanyon_Combined_Field(angles, a_init=1.0, b_init=10.0)
    ...
    def direct_velocity_prediction(self, x, x_star):
        ...
        # 映射到潜在空间 - 使用可微分版本
        combined = torch.cat([x, x_star], dim=0)  # 形状为 [n+1, 2]
        y_combined = self.diffeo.forward(combined)
        y = y_combined[:x.size(0)]
        y_star = y_combined[x.size(0):]
        ...
        # 一次性计算所有扰动的输出
        outputs_batch = self.diffeo.forward(perturbations)
        # 拆分结果并计算雅可比矩阵
        for i in range(dim):
            # 获取正扰动和负扰动的输出
