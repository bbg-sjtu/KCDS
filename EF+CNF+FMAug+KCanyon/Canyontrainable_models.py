# 首先使用标准场训练得到映射模型
# 加载已经训练好的模型，实例化KCanyonCombField，实例化一个新模型，在此基础上开展数据增强训练。
...
import time
from KCanyonPotential import velocity_field_vectorized
from YspaceKmeans import yspace_kmeans
from KCanyonCombField import KCanyon_Combined_Field
...
class SDSEF_FM_KCanyonTrainable(nn.Module):
    """
    使用流匹配的欧几里得化流的稳定动力系统
    """
    def __init__(self, dim, hidden_dim, num_layers, K, SDSEF_FM_modelpath, x0s, x_star):
        super(SDSEF_FM_KCanyonTrainable, self).__init__()
        self.dim = dim
        self.K = K
        self.diffeo = FlowMatchingDiffeo(dim, hidden_dim, num_layers).to(device)
        self.diffeo.velocity_field.load_state_dict(torch.load(SDSEF_FM_modelpath))

        self.initKCanyonField(x0s, x_star)
    
    def initKCanyonField(self,x0s,x_star):
        # 将x0和x*映射到y空间
        self.y0s = self.diffeo.differentiable_forward(x0s).detach().cpu().numpy()
        self.y_star = self.diffeo.differentiable_forward(x_star).detach().cpu().numpy()
        # 在y空间进行kmeans聚类
        self.angles, self.centroids = yspace_kmeans(self.y_star, self.y0s, self.K)

        self.CanyonVF = KCanyon_Combined_Field(self.angles, a_init=1.0, b_init=10.0)

    # forward 有待修改，先使用direct_velocity_prediction，2025.8.28
    ...
    def direct_velocity_prediction(self, x, x_star):
        ...
        # 使用KCanyon计算隐空间速度
        diff = y - y_star
        latent_velocity = self.CanyonVF.compute_velocity(diff)
    ...
    def inference(self, x, x_star, dt=0.01, steps=1000):
        # 欧拉积分推理
        traj=[]
        traj.append(x)
        i=0
        while(i<steps):
            ...
            print('velocity time:', time.time()-t1)
            traj.append(x)
            i += 1
            if torch.norm(x - torch.tensor([[0.0,0.0]], device=x.device)) < 1e-3:
                break
        traj_np = np.stack([t.detach().cpu().numpy() if hasattr(t, 'cpu') else np.array(t) for t in traj]).reshape(-1,2)

        return traj_np

    def inference_in_yspace(self, y, y_star, dt=0.05, steps=1000):
        # dt=0.05的总轨迹推理时间需要0.2s左右，已经比较快了
        # 欧拉积分推理
        traj=[]
        traj.append(y)
        i=0
        while(i<steps):
            y = y + dt * self.CanyonVF.compute_velocity(y - y_star)
            traj.append(y)
            i += 1
            if torch.norm(y - y_star) < 1e-3:
                break
        traj = torch.stack(traj).reshape(-1,2)
        return traj

    # 隐空间是KCanyon的chunk预测方法
    def chunkpredict(self, x, x_star):
        ...
        # 在隐空间中积分推理y到y_star（感觉会快点，因为不涉及网络积分，是现成函数计算后的积分）
        y_traj=self.inference_in_yspace(y, y_star)
        x_traj = self.diffeo.differentiable_inverse(y_traj)
        return x_traj