...
class GlobalTrajectoryDataset(Dataset):
    ...
    def boundary_select(self,r1,r2):
        ...
        # # 计算 NLL
        # nll = -self.gmm.score_samples(self.fm_x)
        # # 首先FM生成的数据超过t1边界进入示教区域内部的点全部删除，只保留大于t1边界的值
        # self.fm_x = self.fm_x[nll > t1]
        # self.fm_v = self.fm_v[nll > t1]

        # # 选择在边界附近的点 (阈值 ±10%)
        # nll = -self.gmm.score_samples(self.fm_x)
        # mask = (nll > t1) & (nll < t2)
        # boundary_fm_x = self.fm_x[mask]
        # boundary_fm_v = self.fm_v[mask]
        # boundary_nll = nll[mask]
