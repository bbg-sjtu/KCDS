...
def OTCFM_x_Trainer(file_name):
    ...
    # 保存模型
    np.savetxt(f"models/OTCFM_x/loss_{file_name}.txt", loss_list)
    plot_loss(loss_list, f'visualizations/OTCFM_x/OTCFM_x_loss_{file_name}.png')
    torch.save(model.state_dict(), f'models/OTCFM_x/OTCFM_x_{file_name}.pth')
    print(f"Model saved to models/OTCFM_x/OTCFM_x_{file_name}.pth")
    ...
    plot_trajectories(res_xt, limit, positions,  f'visualizations/OTCFM_x/OTCFM_x_traj_{file_name}.png', plotboundary=True)
    plot_flowfield(model, limit, positions, f'visualizations/OTCFM_x/OTCFM_x_flowfield_{file_name}.png', plotboundary=True, plottraj=False)

if __name__ == "__main__":
    for i in range(len(filename_list)):
        print("Trajectory:", filename_list[i])
        OTCFM_x_Trainer(filename_list[i])